import { useAuth } from '../../lib/auth/auth-context'
import { startGoogleLogin } from './google'

// Google-only sign-in. Backend-driven OAuth: the button does a full-page navigate
// to the backend, which redirects to Google and back; AuthProvider establishes the
// session from the URL fragment on the /auth/callback landing.
export function AuthCard() {
  const { googleError } = useAuth()

  return (
    <div className="w-full max-w-[392px]">
      <h2 className="text-[25px] font-bold tracking-tight">Sign in to GEO</h2>
      <p className="mb-6 mt-1.5 text-[14.5px] text-muted">
        Continue with your Google account to track your AI visibility.
      </p>

      {googleError && (
        <div role="alert" className="mb-4 rounded-lg bg-bad-soft px-3 py-2 text-sm text-bad">
          {googleError}
        </div>
      )}

      <button type="button" onClick={startGoogleLogin}
        className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-line-strong bg-card py-3 font-semibold hover:bg-card-2">
        <svg aria-hidden="true" className="h-[17px] w-[17px]" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.56c2.08-1.92 3.28-4.74 3.28-8.09Z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.56-2.76c-.98.66-2.23 1.06-3.72 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"/><path fill="#FBBC05" d="M5.84 14.11a6.6 6.6 0 0 1 0-4.22V7.05H2.18a11 11 0 0 0 0 9.9l3.66-2.84Z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.05l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"/></svg>
        Continue with Google
      </button>

      <p className="mt-5 text-center text-[12.5px] text-faint">
        We only use Google to sign you in — no passwords to manage.
      </p>
    </div>
  )
}
