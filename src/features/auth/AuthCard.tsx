...
export function AuthCard() {
  ...
  const { login, register: registerUser, googleError, clearGoogleError } = useAuth()
  ...
  return (
    <div className="w-full max-w-[392px]">
      ...
      <div role="tabpanel" id="auth-panel" aria-labelledby={isRegister ? 'register-tab' : 'signin-tab'} tabIndex={0}>
        ...
        {/* Backend-driven Google OAuth: a full-page navigate to the backend, which
            redirects to Google and back (tokens handled on load by AuthProvider). */}
        <button type="button" onClick={startGoogleLogin}
          className="flex w-full items-center justify-center gap-2.5 rounded-xl border border-line-strong bg-card py-3 font-semibold hover:bg-card-2">
